...
function showTitleInfo(title) {
    ...
    let wordsContainer = document.createElement('ul');

    console.log(`The item -- ${title} -- was clicked!`);

    let titleContainer = document.createElement('li');
    titleContainer.classList.add('productTitles');
    wordsContainer.appendChild(titleContainer);
      ...
      fetch(fiveProducts)
        ...
        // data from the response.json here
            .then(data => {
                ...
                parentList.appendChild(titleContainer);

                document.getElementsByClassName('productTitles').innerText = title
